...
# ────────────────────────────────────────────
# 2. 连线配对 Matching（字段名改为 concept / definition）
# ────────────────────────────────────────────
MATCHING_PROMPT = """\
...
{{
  ...
  "pairs": [
    {{
      "concept": "左侧概念（如：mRNA）",
      "definition": "右侧定义（如：携带遗传信息从DNA到核糖体）"
    }}
  ]
}}
"""
...
# ────────────────────────────────────────────
# 3. 排序题 Sorting（改为直接输出 items 数组）
# ────────────────────────────────────────────
SORTING_PROMPT = """\
你是一位教学设计专家。请根据以下【知识点材料】，设计一组排序题。
...
{{
  "title": "排序游戏标题",
  "items": ["步骤1", "步骤2", "步骤3", "步骤4", "步骤5"]
}}

注意：items 中的元素顺序就是正确顺序，游戏会自动打乱后让学生排列。
"""
...
# ────────────────────────────────────────────
# 5. 判断题 True/False（字段名 statement 改为 stem）
# ────────────────────────────────────────────
TRUE_FALSE_PROMPT = """\
...
# ────────────────────────────────────────────
# 7. 概念填图 / 流程图补全（移除 flow 包装，直接输出顶层字段）
# ────────────────────────────────────────────
FLOW_FILL_PROMPT = """\
