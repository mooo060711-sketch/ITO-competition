...
<head>
    ...
    <script>
        tailwind.config = {{
            theme: {{
                extend: {{
                    fontFamily: {{ sans: ['Nunito', 'sans-serif'] }},  // ✅ 修复语法错误
                    colors: {{
...
def render_quiz(data: dict) -> str:
    ...
    <script>
        ...
        // 防御性检查：确保 questions 是数组且不为空
        if (!Array.isArray(questions) || questions.length === 0) {{
            document.getElementById('quiz-container').innerHTML = '<div class="text-center text-red-500 p-10">题目数据加载失败，请稍后重试。</div>';
...
def render_matching(data: dict) -> str:
    ...
    <script>
        ...
        if (!Array.isArray(pairs) || pairs.length === 0) {{
            document.getElementById('match-container').innerHTML = '<div class="text-center text-red-500 p-10">配对数据加载失败。</div>';
        ...
        // 兼容多种字段名：优先使用 concept/definition，同时支持 left/right/key/value/term/name/def/description 等
        const leftItems = shuffle(pairs.map((p, i) => ({{
            id: i,
            text: p.concept || p.left || p.key || p.term || p.name || "未知概念"
        }})));
        const rightItems = shuffle(pairs.map((p, i) => ({{
            id: i,
            text: p.definition || p.right || p.value || p.def || p.description || "未知定义"
        }})));
...
def render_sorting(data: dict) -> str:
    ...
    <script>
        ...
        if (!Array.isArray(correctItems) || correctItems.length === 0) {{
            document.getElementById('sortable-list').innerHTML = '<li class="text-center text-red-500 p-10">排序数据加载失败。</li>';
            throw new Error('排序数据为空');
        }}
...
def render_fillblank(data: dict) -> str:
    ...
    <script>
        const questions = {q_json};
        if (!Array.isArray(questions) || questions.length === 0) {{
            document.getElementById('fb-container').innerHTML = '<div class="text-center text-red-500 p-10">填空数据加载失败。</div>';
            throw new Error('填空数据为空');
        }}
...
def render_tf(data: dict) -> str:
    ...
    <script>
        const questions = {q_json};
        if (!Array.isArray(questions) || questions.length === 0) {{
            document.getElementById('tf-card').innerHTML = '<div class="text-center text-red-500 p-10">判断数据加载失败。</div>';
            throw new Error('判断数据为空');
        }}
...
def render_flashcard(data: dict) -> str:
    ...
    <script>
        ...
        if (!Array.isArray(cards) || cards.length === 0) {{
            document.getElementById('fc-inner').innerHTML = '<div class="text-center text-red-500 p-10">闪卡数据加载失败。</div>';
            throw new Error('闪卡数据为空');
        }}
...
def render_flowfill(data: dict) -> str:
    ...
    # 直接构造包含 nodes, arrows, blank_answers 的字典
    flow_data = {
    ...
    <script>
        ...
        if (nodes.length === 0 || Object.keys(blanksDict).length === 0) {{
            document.getElementById('flow-container').innerHTML = '<div class="text-center text-red-500 p-10">流程图数据加载失败。</div>';
            throw new Error('流程图数据不完整');
        }}
        ...
        function render() {{
            ...
            optionsList.forEach(opt => {{
                ...
                if(!isUsed) {{
                    btn.onclick = () => {{
                        selectedOpt = selectedOpt === opt? null : opt;
                        render();
                    }};
                }}
    ...
    """ + _FOOTER
    return html_content
def safe_render_wrapper(render_func, data: dict):
    """
    通用防御层：确保注入 HTML 的数据是经过清洗且键名对齐的，防止前端崩溃
    """
    try:
        # 1. 针对排序游戏 (Sorting)：确保 items 是纯文本数组
        if isinstance(data, dict) and "items" in data:
            cleaned_items = []
            for item in data["items"]:
                if isinstance(item, dict):
                    # 尝试提取文本内容，避免渲染成 [object Object]
                    cleaned_items.append(item.get("content", item.get("text", str(item))))
                else:
                    cleaned_items.append(str(item))
            data["items"] = cleaned_items
        
        # 2. 针对配对游戏 (Matching)：从 pairs 数组中归一化键名
        if isinstance(data, dict) and "pairs" in data:
            cleaned_pairs = []
            for pair in data["pairs"]:
                if isinstance(pair, dict):
                    # 强制规整为前端需要的 concept / definition
                    c = pair.get("concept", pair.get("term", pair.get("left", pair.get("key", "未知概念"))))
                    d = pair.get("definition", pair.get("right", pair.get("value", "未知定义")))
                    cleaned_pairs.append({"concept": c, "definition": d})
            data["pairs"] = cleaned_pairs
            
        return render_func(data)
    except Exception as e:
        return f"<div style='text-align:center; color:#dc2626; padding:2.5rem; border:2px solid #fca5a5; background:#fef2f2; border-radius:1rem; margin:1rem;'>引擎渲染层防御拦截到异常: {str(e)}</div>"

# 统一导出渲染器字典
GAME_RENDERERS = {
    "quiz": render_quiz,
    "matching": lambda d: safe_render_wrapper(render_matching, d),
    "sorting": lambda d: safe_render_wrapper(render_sorting, d),
