...
/**
 * Store an arbitrary file as a Blob in IndexedDB.
 * Returns a storage key that can be used to retrieve the blob later.
 */
export async function storeFileBlob(file: File, prefix: string = 'file'): Promise<string> {
  const storageKey = `${prefix}_${nanoid(10)}`;
...
/**
 * Load a generic file Blob from IndexedDB by its storage key.
 */
