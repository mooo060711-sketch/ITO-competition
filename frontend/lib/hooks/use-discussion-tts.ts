...
export function useDiscussionTTS({ enabled, agents, onAudioStateChange }: DiscussionTTSOptions) {
  ...
  const processQueue = useCallback(async () => {
    ...
    try {
      ...
      if (!aborted) {
        console.error('[DiscussionTTS] TTS generation failed, fallback to browser TTS:', err);
        // Graceful fallback: keep discussion flowing even when server TTS provider is unstable.
        onAudioStateChangeRef.current?.(item.agentId, 'playing');
        isPlayingRef.current = true;
        browserSpeakRef.current(item.text);
        return;
      }
      isPlayingRef.current = false;
      onAudioStateChangeRef.current?.(item.agentId, 'idle');
