...
const PREFERRED_LLM_PROVIDER = 'qwen' as ProviderId;
const PREFERRED_LLM_MODEL = 'qwen3.5-plus';
const PREFERRED_TTS_PROVIDER = 'qwen-tts' as TTSProviderId;
const PREFERRED_ASR_PROVIDER = 'qwen-asr' as ASRProviderId;
const PREFERRED_IMAGE_PROVIDER = 'qwen-image' as ImageProviderId;
const PREFERRED_IMAGE_MODEL = 'qwen-image';
...
const getDefaultAudioConfig = () => ({
  ttsProviderId: PREFERRED_TTS_PROVIDER,
  ttsVoice: 'Cherry',
  ttsSpeed: 1.0,
  asrProviderId: PREFERRED_ASR_PROVIDER,
  asrLanguage: 'zh',
  ttsProvidersConfig: {
  ...
  asrProvidersConfig: {
    ...
    'qwen-asr': { apiKey: '', baseUrl: '', enabled: true },
...
const getDefaultImageConfig = () => ({
  imageProviderId: PREFERRED_IMAGE_PROVIDER,
  imageModelId: PREFERRED_IMAGE_MODEL,
...
const getDefaultVideoConfig = () => ({
  ...
  videoProvidersConfig: {
    seedance: { apiKey: '', baseUrl: '', enabled: true },
  ...
  persist(
    (set) => {
      ...
      return {
        ...
        providerId: migratedData?.providerId || PREFERRED_LLM_PROVIDER,
        modelId: migratedData?.modelId || PREFERRED_LLM_MODEL,
        providersConfig: migratedData?.providersConfig || getDefaultProvidersConfig(),
        ttsModel: migratedData?.ttsModel || PREFERRED_TTS_PROVIDER,
        ...
        fetchServerProviders: async () => {
          try {
            ...
            set((state) => {
              ...
              const serverHasPreferredLlm = !!newProvidersConfig[PREFERRED_LLM_PROVIDER]
                ?.isServerConfigured;
              const serverHasPreferredTts = !!newTTSConfig[PREFERRED_TTS_PROVIDER]
                ?.isServerConfigured;
              const serverHasPreferredAsr = !!newASRConfig[PREFERRED_ASR_PROVIDER]
                ?.isServerConfigured;
              const serverHasPreferredImage = !!newImageConfig[PREFERRED_IMAGE_PROVIDER]
                ?.isServerConfigured;

              if (
                serverHasPreferredTts &&
                (!state.autoConfigApplied ||
                  state.ttsProviderId === 'browser-native-tts' ||
                  !newTTSConfig[state.ttsProviderId]?.isServerConfigured)
              ) {
                autoTtsProvider = PREFERRED_TTS_PROVIDER;
                autoTtsVoice = DEFAULT_TTS_VOICES[PREFERRED_TTS_PROVIDER] || 'default';
              }

              if (
                serverHasPreferredAsr &&
                (!state.autoConfigApplied ||
                  state.asrProviderId === 'browser-native' ||
                  !newASRConfig[state.asrProviderId]?.isServerConfigured)
              ) {
                autoAsrProvider = PREFERRED_ASR_PROVIDER;
              }

              if (!state.autoConfigApplied) {
                // PDF: unpdf → mineru if server has it

                // TTS: prefer Qwen, otherwise select the first server provider
                ...
                // ASR: prefer Qwen, otherwise select the first server provider
                const serverAsrIds = Object.keys(data.asr) as ASRProviderId[];
                if (serverHasPreferredAsr && !autoAsrProvider) {
                  autoAsrProvider = PREFERRED_ASR_PROVIDER;
                } else if (
                ...
                // Image: prefer Qwen, otherwise select the first server provider
                const serverImageIds = Object.keys(data.image) as ImageProviderId[];
                if (
                  serverHasPreferredImage &&
                  !newImageConfig[state.imageProviderId]?.isServerConfigured
                ) {
                  autoImageProvider = PREFERRED_IMAGE_PROVIDER;
                  autoImageModel = PREFERRED_IMAGE_MODEL;
                } else if (
                ...
                const serverHasSeedance = !!newVideoConfig.seedance?.isServerConfigured;
                if (
                  serverHasSeedance &&
                  (!newVideoConfig[state.videoProviderId]?.isServerConfigured ||
                    state.videoProviderId !== 'seedance')
                ) {
                  autoVideoProvider = 'seedance' as VideoProviderId;
                  autoVideoModel = 'doubao-seedance-1-5-pro-251215';
                } else if (
                ...
                if (
                  (serverHasSeedance || serverVideoIds.length > 0) &&
              ...
              // LLM auto-select: prefer Qwen when available
              let autoProviderId: ProviderId | undefined;
              let autoModelId: string | undefined;
              if (!state.modelId || state.providerId === 'openai') {
                if (serverHasPreferredLlm) {
                  autoProviderId = PREFERRED_LLM_PROVIDER;
                  autoModelId =
                    newProvidersConfig[PREFERRED_LLM_PROVIDER]?.serverModels?.includes(
                      PREFERRED_LLM_MODEL,
                    )
                      ? PREFERRED_LLM_MODEL
                      : newProvidersConfig[PREFERRED_LLM_PROVIDER]?.serverModels?.[0] ||
                        PROVIDERS[PREFERRED_LLM_PROVIDER]?.models.find(
                          (model) => model.id === PREFERRED_LLM_MODEL,
                        )?.id ||
                        PROVIDERS[PREFERRED_LLM_PROVIDER]?.models[0]?.id;
                }
              }
              if (!autoModelId && !state.modelId) {
    ...
    },
    {
      ...
      migrate: (persistedState: unknown, version: number) => {
        ...
        // v2 → v3: Switch default TTS from browser-native to qwen-tts (LLM TTS)
        if (version < 3) {
          if (state.ttsProviderId === 'browser-native-tts') {
            state.ttsProviderId = 'qwen-tts' as TTSProviderId;
            state.ttsVoice = 'Cherry';
          }
          // Enable qwen-tts by default
          if (state.ttsProvidersConfig) {
            const qwenConfig = state.ttsProvidersConfig['qwen-tts' as TTSProviderId];
            if (qwenConfig) {
              qwenConfig.enabled = true;
            }
          }
        }

        // v3 -> v4: prefer the Qwen demo stack over generic/browser defaults
        if (version < 4) {
          if (
            !state.modelId ||
            state.providerId === 'openai'
          ) {
            state.providerId = PREFERRED_LLM_PROVIDER;
            state.modelId = PREFERRED_LLM_MODEL;
          }

          if (state.ttsModel === 'openai-tts') {
            state.ttsModel = PREFERRED_TTS_PROVIDER;
          }

          if (state.ttsProviderId === 'openai-tts' || state.ttsProviderId === 'browser-native-tts') {
            state.ttsProviderId = PREFERRED_TTS_PROVIDER;
            state.ttsVoice = DEFAULT_TTS_VOICES[PREFERRED_TTS_PROVIDER] || 'Cherry';
          }

          if (state.asrProviderId === 'browser-native') {
            state.asrProviderId = PREFERRED_ASR_PROVIDER;
          }

          if (
            state.imageProviderId === 'seedream' &&
            (!state.imageModelId || state.imageModelId === 'doubao-seedream-5-0-260128')
          ) {
            state.imageProviderId = PREFERRED_IMAGE_PROVIDER;
            state.imageModelId = PREFERRED_IMAGE_MODEL;
          }

          if (state.asrProvidersConfig?.[PREFERRED_ASR_PROVIDER]) {
            state.asrProvidersConfig[PREFERRED_ASR_PROVIDER].enabled = true;
          }

          if (state.imageProvidersConfig?.[PREFERRED_IMAGE_PROVIDER]) {
            state.imageProvidersConfig[PREFERRED_IMAGE_PROVIDER].enabled = true;
          }
        }

        // v4 -> v5: enable the default Seedance video stack for demo mode
        if (version < 5) {
          if (
            !state.videoModelId ||
            (state.videoProviderId === 'seedance' &&
              state.videoModelId === 'doubao-seedance-1-5-pro-251215')
          ) {
          ...
          if (state.videoProvidersConfig?.seedance) {
            state.videoProvidersConfig.seedance.enabled = true;
          }
        }
