...
function normalizeAudioFormat(format?: string): 'webm' | 'm4a' | 'mp3' | 'wav' | 'flac' | 'ogg' {
  const v = (format || '').trim().toLowerCase();
  if (v === 'webm') return 'webm';
  if (v === 'm4a' || v === 'mp4') return 'm4a';
  if (v === 'mp3' || v === 'mpeg' || v === 'mpga') return 'mp3';
  if (v === 'wav') return 'wav';
...
function toDataUrlMime(format: 'webm' | 'm4a' | 'mp3' | 'wav' | 'flac' | 'ogg'): string {
...
function isRetryableFetchError(error: unknown): boolean {
  ...
  return /fetch failed|network|timeout|timed out|socket|econn|aborted/i.test(message);
}
...
async function transcribeQwenASR(
  ...
  const audioFormat = normalizeAudioFormat(config.format);
  const audioMime = toDataUrlMime(audioFormat);
      ...
      messages: [
        {
          ...
          content: [
            {
              audio: `data:${audioMime};base64,${base64Audio}`,
            },
          ],
        },
      ],
  ...
  const response = await fetchWithRetry(`${baseUrl}/services/aigc/multimodal-generation/generation`, {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${config.apiKey}`,
      'Content-Type': 'application/json; charset=utf-8',
      'X-DashScope-Audio-Format': audioFormat,
    },
  ...
  if (!response.ok) {
    ...
    throw new Error(`Qwen ASR API error (${audioFormat}): ${errorText}`);
  }
