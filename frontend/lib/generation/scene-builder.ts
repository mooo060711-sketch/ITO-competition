...
import { matchBiologyImage, getBiologyImageUrl } from '@/lib/utils/biology-images';
...
export function buildCompleteScene(
  ...
  // Fallback: content format didn't match any known type (e.g. backend proxy artifacts).
  // Build a basic slide scene so generation doesn't fail entirely.
  log.warn(`Unknown content format for "${outline.title}" (type=${outline.type}), using fallback slide`);

  const matchedImage = matchBiologyImage(outline.title, outline.description);
  const hasImage = !!matchedImage;

  const fallbackElements: Slide['elements'] = [
    {
      id: nanoid(),
      type: 'text',
      left: hasImage ? 40 : 50,
  ...
  if (hasImage) {
    fallbackElements.push({
      ...
      width: 420,
      height: 440,
      src: getBiologyImageUrl(matchedImage!),
      rotate: 0,
      fixedRatio: true,
    });
  }
  ...
  const fallbackSlide: Slide = {
    ...
    background: { type: 'solid', color: '#f0faf4' },
  };
