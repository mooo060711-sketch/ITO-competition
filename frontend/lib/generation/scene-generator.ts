...
> {
  if (outline.type === 'interactive' && !outline.interactiveConfig) {
    log.warn(`Interactive outline "${outline.title}" missing interactiveConfig, auto-completing it`);
    outline = {
      ...outline,
      interactiveConfig: buildInteractiveConfigFromOutline(outline),
    };
  }
...
async function generateSlideContent(
  ...
  const effectiveAssignedImages =
    assignedImages && assignedImages.length > 0 && !visionEnabled ? undefined : assignedImages;

  if (assignedImages && assignedImages.length > 0 && !visionEnabled) {
    log.warn(
      `Vision unavailable for "${outline.title}", skipping ${assignedImages.length} assigned PDF image(s).`,
    );
  }
  ...
  if (effectiveAssignedImages && effectiveAssignedImages.length > 0) {
    if (visionEnabled && imageMapping) {
      // Vision mode: split into vision images and text-only
      const withSrc = effectiveAssignedImages.filter((img) => imageMapping[img.id]);
      const visionSlice = withSrc.slice(0, MAX_VISION_IMAGES);
      const textOnlySlice = withSrc.slice(MAX_VISION_IMAGES);
      const noSrcImages = effectiveAssignedImages.filter((img) => !imageMapping[img.id]);
      ...
      assignedImagesText = effectiveAssignedImages
  ...
  if (effectiveAssignedImages && effectiveAssignedImages.length > 0) {
    log.debug(`Assigned images: ${effectiveAssignedImages.map((img) => img.id).join(', ')}`);
  }
  ...
  const fixedElements = fixElementDefaults(generatedData.elements, effectiveAssignedImages);
...
function getInteractiveConfig(outline: SceneOutline): NonNullable<SceneOutline['interactiveConfig']> {
  if (outline.interactiveConfig) {
    return outline.interactiveConfig;
  }

  log.warn(`Interactive outline "${outline.title}" missing interactiveConfig, using auto-completed config`);
  return buildInteractiveConfigFromOutline(outline);
}

function buildInteractiveConfigFromOutline(
  outline: SceneOutline,
): NonNullable<SceneOutline['interactiveConfig']> {
  const keyPointText = (outline.keyPoints || [])
    .map((point) => point.trim())
    .filter(Boolean)
    .slice(0, 4)
    .join('；');
  const conceptOverview = outline.description?.trim() || keyPointText || outline.title;
  const subject = inferSubjectFromText(
  ...
  return {
    ...
    conceptOverview,
    designIdea: `围绕“${outline.title}”构建与本节课内容强相关的互动卡牌任务：每张卡牌对应关键知识点，用户通过点击/拖拽/排序完成判定，页面实时反馈得分与纠错提示。`,
    subject,
  };
}
...
function inferSubjectFromText(text: string): string {
  ...
  if (/(生物|遗传|dna|基因|细胞|酶|蛋白|中心法则)/i.test(normalized)) return 'Biology';
  if (/(化学|分子|离子|反应|酸碱|元素)/i.test(normalized)) return 'Chemistry';
  if (/(物理|力学|电路|速度|加速度|能量|波)/i.test(normalized)) return 'Physics';
  if (/(数学|函数|几何|概率|方程|导数|积分)/i.test(normalized)) return 'Mathematics';
  if (/(程序|算法|数据结构|编程|代码)/i.test(normalized)) return 'Computer Science';
...
async function generateInteractiveContent(
  ...
  const config = getInteractiveConfig(outline);
  ...
  const relevanceKeywords = collectInteractiveRelevanceKeywords(outline, config);
  const minKeywordMatches =
    relevanceKeywords.length === 0 ? 0 : Math.min(relevanceKeywords.length, relevanceKeywords.length >= 3 ? 3 : 2);
  const maxAttempts = 3;
  let feedbackForRetry = '';

  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    const strictPrompt = buildInteractiveStrictPrompt({
      basePrompt: htmlPrompts.user,
      relevanceKeywords,
      minKeywordMatches,
      retryFeedback: feedbackForRetry,
      attempt,
    });

    log.info(`Step 2: Generating HTML for: ${outline.title} (attempt ${attempt}/${maxAttempts})`);
    const htmlResponse = await aiCall(htmlPrompts.system, strictPrompt);
    const rawHtml = extractHtml(htmlResponse);
    if (!rawHtml) {
      feedbackForRetry = '输出中缺少完整 HTML 文档结构（含 <html> 与 </html>）。';
      log.warn(`Interactive HTML extraction failed for "${outline.title}" on attempt ${attempt}`);
      continue;
    }

    const processedHtml = postProcessInteractiveHtml(rawHtml);
    const validation = validateInteractiveHtml({
      html: processedHtml,
      relevanceKeywords,
      minKeywordMatches,
    });
    ...
    if (validation.valid) {
      log.info(
        `Interactive HTML validated (${processedHtml.length} chars, keyword hits: ${validation.keywordHits.length}) for: ${outline.title}`,
      );
      return {
        html: processedHtml,
        scientificModel,
    ...
    feedbackForRetry = validation.issues.join('；');
    log.warn(
      `Interactive HTML validation failed for "${outline.title}" on attempt ${attempt}: ${feedbackForRetry}`,
    );
  }

  log.error(
    `Failed to generate valid interactive HTML for "${outline.title}" after ${maxAttempts} attempts`,
  );
  return null;
}
...
function buildInteractiveStrictPrompt(params: {
  basePrompt: string;
  ...
  const { basePrompt, relevanceKeywords, minKeywordMatches, retryFeedback, attempt } = params;
  const keywordsText =
    relevanceKeywords.length > 0
      ? relevanceKeywords.join('、')
      : '使用本场景标题、描述与关键点中的核心术语';

  const retryLine = retryFeedback
    ? `\n- 上次输出不通过原因：${retryFeedback}\n- 请完整重写页面，不要做局部修补。`
    : '';
...
## Strict Validation Gate (MUST PASS)

1. 页面内容必须与本场景强相关，不可输出通用模板或空泛逻辑训练题。
2. 页面需至少包含 3 个用户可操作控件，并包含事件驱动的状态更新。
3. 页面必须有实时反馈区（得分/进度/判定结果），且每次交互后都会更新。
4. 请优先采用卡牌机制（翻牌/配对/拖拽排序/卡组选择均可），每张卡牌内容需来源于本场景知识点。
5. 下列关键词至少命中 ${minKeywordMatches} 个：${keywordsText}
6. 严禁出现“兜底互动流程”“自动互动增强模式”或任何通用兜底文案。${retryLine}

当前为第 ${attempt} 次生成，请一次性输出可直接运行的完整 HTML 文档。`;
}

function collectInteractiveRelevanceKeywords(
  outline: SceneOutline,
  config: NonNullable<SceneOutline['interactiveConfig']>,
): string[] {
  const candidates = [
    outline.title,
    outline.description,
  ...
  for (const candidate of candidates) {
    const terms = tokenizeRelevanceText(candidate);
...
function validateInteractiveHtml(params: {
  html: string;
  relevanceKeywords: string[];
  minKeywordMatches: number;
}): { valid: boolean; issues: string[]; keywordHits: string[] } {
  const { html, relevanceKeywords, minKeywordMatches } = params;
  const lower = html.toLowerCase();
  const visibleText = extractVisibleTextFromHtml(html).toLowerCase();
  const issues: string[] = [];

  const controlTagCount = (html.match(/<(button|input|select|textarea|details)\b/gi) || []).length;
  const editableCount = (html.match(/\bcontenteditable\s*=/gi) || []).length;
  const draggableCount = (html.match(/\bdraggable\s*=\s*["']?true["']?/gi) || []).length;
  const controlCount = controlTagCount + editableCount + draggableCount;

  if (controlCount < 3) {
    issues.push(`可操作控件不足（检测到 ${controlCount} 个，要求至少 3 个）`);
  }

  const hasBehavior = [
    /addEventListener\s*\(/i,
    /on(click|input|change|drag|drop|pointer|mousedown|mouseup|touchstart)\s*=/i,
    /\.onclick\s*=/i,
    /requestAnimationFrame\s*\(/i,
  ].some((pattern) => pattern.test(html));
  if (!hasBehavior) {
    issues.push('缺少交互事件处理逻辑（未检测到事件监听）');
  }

  const hasFeedbackVocabulary =
    /(score|progress|result|feedback|correct|wrong|得分|进度|结果|反馈|正确|错误|答对|答错|完成度)/i.test(
      lower,
    );
  const hasDomUpdates =
    /(textcontent\s*=|innertext\s*=|innerhtml\s*=|classlist\.(add|remove|toggle)\(|setattribute\s*\(|style\.[a-z-]+\s*=)/i.test(
      lower,
    );
  if (!hasFeedbackVocabulary || !hasDomUpdates) {
    issues.push('缺少“交互后即时反馈”机制（反馈文案或状态更新不足）');
  }

  const hasCardMechanic = /(card|deck|flip|match|drag|drop|卡牌|翻牌|配对|拖拽|卡组)/i.test(lower);
  if (!hasCardMechanic) {
    issues.push('未检测到卡牌或等价互动机制关键词（建议使用卡牌玩法）');
  }

  if (/兜底互动流程|自动互动增强模式|generic fallback|fallback interactive/i.test(lower)) {
    issues.push('包含禁止的兜底模板文案');
  }

  const keywordHits = relevanceKeywords.filter((keyword) => {
    const normalized = normalizeKeyword(keyword);
    if (!normalized) return false;
    return lower.includes(normalized) || visibleText.includes(normalized);
  });

  if (relevanceKeywords.length > 0 && keywordHits.length < minKeywordMatches) {
    issues.push(
      `与本场景内容关联不足（关键词命中 ${keywordHits.length}/${Math.min(relevanceKeywords.length, minKeywordMatches)}）`,
    );
  }
