...
export async function POST(
  ...
  try {
    ...
    const result = await proxyJsonPost(`/v1/jobs/${encodeURIComponent(jobId)}/cancel`, {}, {
      signal: req.signal,
      timeout: 15_000,
    });
    return apiSuccess(result as Record<string, unknown>);
  } catch (err) {
    return apiError('UPSTREAM_ERROR', 500, err instanceof Error ? err.message : 'Cancel failed');
  }
}
