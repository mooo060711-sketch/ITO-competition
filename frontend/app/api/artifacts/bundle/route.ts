...
export async function GET(request: NextRequest) {
  ...
  try {
    const url = getBackendUrl(
      `/v1/artifacts/bundle?session_id=${encodeURIComponent(sessionId)}${
    ...
    return new NextResponse(buffer, {
      ...
      headers: {
        ...
        'Content-Disposition': `attachment; filename="courseware-bundle-${sessionId}${versionId ? `-${versionId}` : ''}.zip"`,
      },
    });
  } catch (err) {
