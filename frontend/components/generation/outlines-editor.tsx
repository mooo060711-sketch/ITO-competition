    ...
    <div className="space-y-6">
        ...
        {outlines.map((outline, index) => (
          <Card key={outline.id} className="relative">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                <Label>{t('outlineEditor.keyPointsLabel')}</Label>
                <Textarea
                  value={outline.keyPoints?.join('\n') || ''}
                  onChange={(e) => updateKeyPoints(index, e.target.value)}
                  placeholder={t('outlineEditor.keyPointsPlaceholder')}
                  rows={3}
                  disabled={isLoading}
                />
              </div>
              ...
              {outline.type === 'quiz' && (
                <div className="p-3 bg-muted/50 rounded-lg space-y-3">
                  <Label className="text-sm font-medium">{t('outlineEditor.quizConfig')}</Label>
                  <div className="grid grid-cols-3 gap-3">
                    <div className="space-y-1">
                      <Label className="text-xs">{t('outlineEditor.questionCount')}</Label>
                      <Input
                        type="number"
                    ...
                    <div className="space-y-1">
                      <Label className="text-xs">{t('outlineEditor.difficulty')}</Label>
                      <Select
                      ...
                      >
                        ...
                        <SelectContent>
                          <SelectItem value="easy">{t('outlineEditor.difficultyEasy')}</SelectItem>
                          <SelectItem value="medium">{t('outlineEditor.difficultyMedium')}</SelectItem>
                          <SelectItem value="hard">{t('outlineEditor.difficultyHard')}</SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                    <div className="space-y-1">
                      <Label className="text-xs">{t('outlineEditor.questionType')}</Label>
                      <Select
                      ...
                      >
                        ...
                        <SelectContent>
                          <SelectItem value="single">{t('outlineEditor.typeSingle')}</SelectItem>
                          <SelectItem value="multiple">{t('outlineEditor.typeMultiple')}</SelectItem>
                          <SelectItem value="text">{t('outlineEditor.typeText')}</SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                  </div>
      ...
      {outlines.length === 0 && (
        <Card className="p-8 text-center">
          <p className="text-muted-foreground mb-4">{t('outlineEditor.noOutlines')}</p>
          <Button variant="outline" onClick={addOutline} disabled={isLoading}>
            <Plus className="size-4 mr-1" />
            {t('outlineEditor.addFirst')}
          </Button>
        </Card>
      )}
      ...
      <div className="flex justify-between pt-4">
        <Button variant="outline" onClick={onBack} disabled={isLoading}>
          {t('outlineEditor.backToEdit')}
        </Button>
        <Button onClick={onConfirm} disabled={isLoading || outlines.length === 0}>
          {isLoading ? t('outlineEditor.generating') : t('outlineEditor.confirmGenerate')}
        </Button>
      </div>
    </div>
  );
