...
const LIVE_SESSION_FALLBACK_AGENT_IDS = ['default-1', 'default-2', 'default-3'];

function normalizeLiveSessionAgentIds(agentIds: string[], sessionType: SessionType): string[] {
  const deduped = Array.from(new Set((agentIds || []).filter((id) => typeof id === 'string' && !!id)));
  if (sessionType !== 'qa' && sessionType !== 'discussion') {
    return deduped.length > 0 ? deduped : ['default-1'];
  }

  const registry = useAgentRegistry.getState();
  const roleOf = (id: string): string | null => registry.getAgent(id)?.role || null;
  const next = [...deduped];

  const addIfMissing = (id: string | undefined) => {
    if (!id) return;
    if (!next.includes(id)) next.push(id);
  };

  const hasTeacher = next.some((id) => roleOf(id) === 'teacher');
  const hasNonTeacher = next.some((id) => {
  ...
  if (!hasTeacher) {
    const fallbackTeacher =
      LIVE_SESSION_FALLBACK_AGENT_IDS.find((id) => roleOf(id) === 'teacher') || 'default-1';
    addIfMissing(fallbackTeacher);
  }

  if (!hasNonTeacher) {
    const fallbackNonTeacher =
      LIVE_SESSION_FALLBACK_AGENT_IDS.find((id) => {
        const role = roleOf(id);
        return role != null && role !== 'teacher';
      }) || 'default-2';
    addIfMissing(fallbackNonTeacher);
  }

  const minAgents = sessionType === 'discussion' ? 3 : 2;
  for (const fallbackId of LIVE_SESSION_FALLBACK_AGENT_IDS) {
    if (next.length >= minAgents) break;
    addIfMissing(fallbackId);
  }
...
export function useChatSessions(options: UseChatSessionsOptions = {}) {
  ...
  const runAgentLoop = useCallback(
    async (
      ...
      const normalizedAgentIds = normalizeLiveSessionAgentIds(
        requestTemplate.config.agentIds,
        sessionType,
      );
      requestTemplate.config.agentIds = normalizedAgentIds;

      // Attach full configs for all selected agents so backend mode can:
      // 1) keep persona consistency per role,
      // 2) route assistant/student/teacher to different provider APIs.
      const registry = useAgentRegistry.getState();
      const resolvedAgentConfigs = normalizedAgentIds
        .map((id: string) => registry.getAgent(id))
        .filter((agent): agent is NonNullable<typeof agent> => Boolean(agent))
        .map((agent) => {
          const llm = resolveRoleLLMConfig({
            role: agent.role,
            providerId: settingsState.providerId,
            modelId: settingsState.modelId,
          ...
          const { createdAt: _c, updatedAt: _u, isDefault: _d, ...rest } = agent;
          return {
            ...rest,
            llm: {
      ...
      if (resolvedAgentConfigs.length > 0) {
        requestTemplate.config.agentConfigs = resolvedAgentConfigs;
      }

      const defaultMaxTurns = normalizedAgentIds.length <= 1 ? 1 : 10;
