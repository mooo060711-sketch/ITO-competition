...
class ReferenceHandler:
    ...
    def _parse_video(self, fp: Path):
        ...
        try:
            ...
            asr_text = transcribe_audio(str(fp))
            if asr_text and len(str(asr_text).strip()) > 10:
                text_content = str(asr_text) if isinstance(asr_text, str) else getattr(asr_text, 'text', str(asr_text))
                parts.append(f"[语音转写]\n{text_content}")
                meta["asr"] = True
        except Exception as e:
            ...
            if has_ffmpeg():
                ...
                with tempfile.TemporaryDirectory() as tmp_dir:
                    frames = extract_frames_with_ffmpeg(
                        str(fp), out_dir=tmp_dir,
                        interval_sec=10, max_frames=5,
                    )
                    if frames:
                        ...
                        if vlm_cfg.get("enabled"):
                            try:
                                from ..tools.vlm import describe_image
                                for i, frame_path in enumerate(frames[:3]):  # 最多描述3帧
                                    desc = describe_image(str(frame_path), vlm_cfg)
                                    if desc:
                                        parts.append(f"[关键帧{i+1}描述]\n{desc}")
                                meta["vlm_frames"] = True
                            except Exception as ve:
                                meta["vlm_error"] = str(ve)
                        else:
                            # 没有VLM就用OCR
                            try:
                                from ..tools.ocr import extract_text_from_image
                                for i, frame_path in enumerate(frames[:3]):
                                    ocr_text = extract_text_from_image(str(frame_path))
                                    if ocr_text and len(ocr_text.strip()) > 10:
                                        parts.append(f"[关键帧{i+1} OCR]\n{ocr_text}")
                            except Exception:
                                pass
        except Exception as e:
        ...
        if parts:
            method = "asr" + ("+vlm_frames" if meta.get("vlm_frames") else
                              "+frames" if meta.get("frames_extracted") else "")
            return "\n\n".join(parts), meta, method
        else:
            return f"视频文件: {fp.name}（ASR和帧提取均未就绪，需要ffmpeg和faster-whisper）", meta, "placeholder"
