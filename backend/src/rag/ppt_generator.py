    ...
    def generate_from_outline(
        ...
        for sd in slides_data:
            ...
            if stype == "cover":
                ...
                # 仅使用上游明确给出的图片路径；不做关键词自动猜图。
                # 这样能保证是否附图由多模态理解链路决定，而不是规则匹配。
                img_path = str(sd.get("image_path") or "").strip()
                if img_path:
                    p = Path(img_path)
                    if not p.is_file():
                        alt = Path("assets/images") / img_path
                        img_path = str(alt) if alt.is_file() else ""
