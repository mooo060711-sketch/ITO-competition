...
class GameEngine:
    ...
    def __init__(self, config_path: str = "config.yaml", llm_router: Optional[LLMRouter] = None):
    ...
    def _call_llm(self, prompt: str) -> str:
        if self._llm_router is not None:
            g_cfg = self.cfg.get("generator", {}) or {}
            return self._llm_router.generate(
                prompt,
                task="game",
                max_tokens=int(g_cfg.get("game_max_new_tokens", 1200)),
                temperature=float(g_cfg.get("temperature", 0.4)),
                timeout_sec=float(g_cfg.get("game_timeout_sec", g_cfg.get("timeout_sec", 40))),
                cache=False,
            )
